 ...
 , gboolean partial, langType lang)
{
	...
	if (matches[0] && *matches[0])
	{
		...
		{
			if ( (type & (*matches[0])->type) 
			  && (lang == -1 || tags_lang == lang)
			 )
			{
			...
			/*
			// this seems redundant, returned tags have been matched by name
			// it also interferes with case insensitive comparison
			if (partial)
			{
