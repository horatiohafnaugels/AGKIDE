...
public:
	...
	bool MatchAGKRem() {
		if (MakeLowerCase(ch) != 'r')
			return false;
		if (MakeLowerCase(chNext) != 'e')
			return false;
		if ('m' != MakeLowerCase(static_cast<unsigned char>(styler.SafeGetCharAt(currentPos+2, 0))))
			return false;

		unsigned char c = static_cast<unsigned char>(styler.SafeGetCharAt(currentPos+3, 0));
