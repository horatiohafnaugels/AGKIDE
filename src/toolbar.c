...
static const GtkActionEntry ui_entries[] = {
	/* custom actions defined in toolbar_init(): "Open", "SearchEntry", "GotoEntry", "Build" */
	{ "New", GTK_STOCK_NEW, NULL, NULL, N_("Create a new project"), G_CALLBACK(on_toolbutton_new_clicked) },
	...
	{ "Compile", "compile", N_("Compile"), NULL, N_("Compile the current project"), G_CALLBACK(on_toolbutton_compile_clicked) },
	{ "Run", "run", N_("Run"), NULL, N_("Run the current project on the local machine"), G_CALLBACK(on_toolbutton_run_clicked) },
	{ "Broadcast", "broadcast", N_("Broadcast"), NULL, N_("Broadcast the current project to devices"), G_CALLBACK(on_toolbutton_broadcast_clicked) },
	{ "Debug", GTK_STOCK_EXECUTE, N_("Debug"), NULL, N_("Debug the current project"), G_CALLBACK(on_toolbutton_debug_clicked) },
...
static void tb_editor_set_item_values(const gchar *name, GtkListStore *store, GtkTreeIter *iter)
{
	...
	else
	{
		...
		g_object_get(action, "stock-id", &icon_stock, NULL);
		//if (icon == NULL)
	...
	gtk_list_store_set(store, iter,
		...
		TB_EDITOR_COL_ICON_STOCK, icon_stock,
		-1);
...
static TBEditorWidget *tb_editor_create_dialog(GtkWindow *parent)
{
	...
	column = gtk_tree_view_column_new_with_attributes(
		NULL, icon_renderer, "icon-name", TB_EDITOR_COL_ICON, "stock-id", TB_EDITOR_COL_ICON_STOCK, NULL);
