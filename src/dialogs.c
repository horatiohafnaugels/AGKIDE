...
static gboolean show_save_as_gtk(GeanyDocument *doc)
{
	...
	if (doc->file_name != NULL)
	{
		...
		{
			if ( strchr( doc->file_name, '.' ) == 0 )
			{
				gchar *fname = NULL;

				if (doc->file_type != NULL && doc->file_type->extension != NULL)
					fname = g_strconcat(doc->file_name, ".", doc->file_type->extension, NULL);
				else
					fname = g_strdup(doc->file_name);
...
static gint run_unsaved_dialog(const gchar *msg, const gchar *msg2)
{
	...
	button = ui_button_new_with_image(GTK_STOCK_CLOSE, _("_Don't save"));
