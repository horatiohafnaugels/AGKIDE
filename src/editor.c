...
static void on_update_ui(GeanyEditor *editor, G_GNUC_UNUSED SCNotification *nt)
{
	...
	// live searching
	if (editor_prefs.highlight_selection)
	{
		...
		if ( selection )
		{
			if ( *selection )
			{
				if ( sci_get_line_from_position( sci, sci_get_selection_start(sci) )
				  == sci_get_line_from_position( sci, sci_get_selection_end(sci) ) )
				{
					struct Sci_TextToFind ttf;
					ttf.chrg.cpMin = 0;
					ttf.chrg.cpMax = sci_get_length(sci);
					ttf.lpstrText = (gchar *)selection;

					GSList *match, *matches;
					matches = find_range(sci, SCFIND_WHOLEWORD, &ttf);
					foreach_slist (match, matches)
					{
						GeanyMatchInfo *info = match->data;

						if (info->end != info->start)
							editor_indicator_set_on_range(editor, GEANY_INDICATOR_SEARCH, info->start, info->end);
...
static void show_tags_list(GeanyEditor *editor, const GPtrArray *tags, gsize rootlen)
{
	...
	if (tags->len > 0)
	{
		...
		for (j = 0; j < tags->len; ++j)
		{
			...
			// remove array sizes from tag name
			strncpy( temp, tag->name, 49 );
			temp[ 49 ] = 0;
			gchar* bracket = strchr(temp, '[');
			if ( bracket ) 
			{
				*bracket = 0;
				bracket--;
				while( isspace(*bracket) && bracket > temp )
				{
					*bracket = 0;
					bracket--;
				}
			}
			g_string_append(words, temp);
...
static void autocomplete_scope(GeanyEditor *editor)
{
	...
	// look for array brackets and ignore them
	if (sci_get_char_at(sci, pos - 2) == ']')
	{
		...
		while ( pos > line_start && sci_get_char_at(sci, pos - 1) != '[' )
			pos--;
	}
...
static gint get_basic_indent(ScintillaObject *sci, gint line)
{
	gboolean need_close = FALSE;
	gint end = get_sci_line_code_end_position(sci, line);
	gint lexer = sci_get_lexer(sci);
	...
	gint style = sci_get_style_at(sci, end);
	if ( !highlighting_is_code_style( lexer, style ) ) return 0;

	gint start = sci_get_position_from_line(sci, line);
	gchar *line_contents = sci_get_contents_range(sci, start, end + 1);

	if ( !line_contents ) return 0;

	gchar *cur = line_contents;
	...
	gchar *endptr = line_contents + (end + 1 - start);
	while ( endptr > cur && isspace(*endptr) ) endptr--;
	if ( endptr <= cur ) 
	{
		g_free(line_contents);
		return 0;
	...
	*endptr = 0;
	...
	gint should_indent = 0;
	gchar* line_lower = utf8_strdown( cur );
	if ( strcmp(line_lower, "do") == 0 ) should_indent = 1;
	if ( strcmp(line_lower, "case default") == 0 ) should_indent = 1;
	if ( strcmp(line_lower, "else") == 0 ) should_indent = 1;
	if ( strcmp(line_lower, "repeat") == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "for ", 4) == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "type ", 5) == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "case ", 5) == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "while ", 6) == 0 || strncmp(line_lower, "while(", 6) == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "select ", 7) == 0 || strncmp(line_lower, "select(", 7) == 0 ) should_indent = 1;
	if ( strncmp(line_lower, "function ", 9) == 0 ) should_indent = 1;

	// special cases that might not indent
	if ( strncmp(line_lower, "if ", 3) == 0 || strncmp(line_lower, "if(", 3) == 0 
	  || strncmp(line_lower, "elseif ", 7) == 0 || strncmp(line_lower, "elseif(", 7) == 0 ) 
	{
		if ( strstr( line_lower, " then " ) == 0 ) should_indent = 1;
	}
...
static gint get_indent_size_after_line(GeanyEditor *editor, gint line)
{
	...
	if (iprefs->auto_indent_mode > GEANY_AUTOINDENT_BASIC)
	{
		...
		{
			if (sci_get_lexer(sci) == SCLEX_PYTHON) /* Python/Cython */
				additional_indent = iprefs->width * get_python_indent(sci, line);
			else if (sci_get_lexer(sci) == SCLEX_AGKBASIC)
				additional_indent = iprefs->width * get_basic_indent(sci, line);
		}
...
autocomplete_tags(GeanyEditor *editor, const gchar *root, gsize rootlen)
{
	...
	tags = tm_workspace_find(root, tm_tag_max_t & ~tm_tag_member_t, attrs, TRUE, doc->file_type->lang);
	if (tags)
	{
...
static gboolean autocomplete_check_html(GeanyEditor *editor, gint style, gint pos)
{
	...
	if (sci_get_lexer(editor->sci) == SCLEX_XML && style < SCE_HJ_START)
		try = TRUE;
...
void editor_apply_update_prefs(GeanyEditor *editor)
{
	...
	editor_set_indent(editor, editor_prefs.indentation->type, editor_prefs.indentation->width);
