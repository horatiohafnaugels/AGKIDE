...
void copy_template_file(const gchar* template_filename, const gchar* new_filename)
{
	GeanyFiletype *ft;
	gchar *template;
	const gchar *extension = strrchr(template_filename, '.'); /* easy way to get the file extension */
	gchar *path;

	ft = filetypes_detect_from_extension(template_filename);
	gchar* fname = utils_get_locale_from_utf8(template_filename);
