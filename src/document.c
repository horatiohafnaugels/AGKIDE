...
GeanyDocument *document_new_file(const gchar *utf8_filename, GeanyFiletype *ft, const gchar *text, gboolean ignore_project)
{
	...
	if (app->project != NULL && !ignore_project )
	{
		...
		{
			if ( dialogs_show_question(_("Do you want to add the new file to the current project '%s'? (This will require you to save the new document somewhere)"), app->project->name) )
			{
				project = app->project;
			}
		}
	...
	if (utf8_filename && g_path_is_absolute(utf8_filename))
	{
		...
		utils_tidy_path(tmp);
		utf8_filename = tmp; // todo does this leak memory?
	}
	else
	{
		if ( !utf8_filename )
		{
			...
			do
			{
				sprintf(tmp, "Untitled%d", i++);
			} while ( sidebar_openfiles_exists( tmp ) );

			utf8_filename = g_strdup( tmp );
		}
	}
	...
	if ( project )
	{
		if ( document_save_file( doc, TRUE ) )
		{
			project_add_file( project, doc->file_name, TRUE );
		}
		else
		{
			//doc->project = NULL;
...
{
	const gchar* ext = strrchr( locale_filename, '.' );
	if ( ext && utils_str_casecmp( ext, ".agk" ) == 0 )
	{
		...
		//if ( dialogs_show_question("That file looks like an AGK project, do you want to load it as a project instead?") )
		{
			...
			if (! project_load_file_with_session(locale_filename))
			{
				gchar *utf8_filename = utils_get_utf8_from_locale(locale_filename);
				dialogs_show_msgbox(GTK_MESSAGE_ERROR, _("Project file \"%s\" could not be loaded."), utf8_filename);
				g_free(utf8_filename);
			}
			return NULL;
		}
		...
		gboolean readonly, GeanyFiletype *ft, const gchar *forced_enc)
{
	...
	{	/* doc possibly changed */
		...
		else
		{
			...
			editor_set_indent( doc->editor, dprefs->type, dprefs->width );
		}
		//else
...
gboolean document_save_file_as(GeanyDocument *doc, const gchar *utf8_fname)
{
	...
	if (utf8_fname != NULL)
	{
		if ( sidebar_openfiles_exists( doc->file_name ) )
		{
			sidebar_remove_document( doc );
		}
...
gboolean document_save_file(GeanyDocument *doc, gboolean force)
{
	...
	// this is a horrible thing to do, don't modify files like this unless the user actions it
...
gboolean document_account_for_unsaved_project(GeanyProject *project)
{
	guint i;
...
static void force_close_all_project(GeanyProject* p)
{
	guint i;

	/* check all documents have been accounted for */
	for (i = 0; i < p->project_files->len; i++)
	{
		if ( !project_files_index(p,i)->is_valid ) 
			continue;

		GeanyDocument *doc = document_find_by_real_path( project_files_index(p,i)->file_name );

		if (DOC_VALID(doc))
		{
			g_return_if_fail(!doc->changed);
		}
	}
	main_status.closing_all = TRUE;
...
gboolean document_close_all_project(GeanyProject *p)
{
	if (! document_account_for_unsaved_project(p))
		return FALSE;

	force_close_all_project(p);
