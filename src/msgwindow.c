...
static const GdkColor *get_color(gint msg_color)
{
	...
	GdkColor *color = NULL;
	GtkStyle *style = gtk_widget_get_style (msgwindow.tree_compiler);

	if (style != NULL) 
		color = &(style->base[GTK_STATE_NORMAL]);

	float lum = 1.0f;
	if ( color ) lum = (color->red*0.21f/65535.0f) + (color->green*0.71f/65535.0f) + (color->blue*0.07f/65535.0f);
...
static GtkWidget *create_message_popup_menu(gint type)
{
	...
	clear = gtk_image_menu_item_new_from_stock(GTK_STOCK_CLOSE, NULL);
...
gboolean msgwin_goto_compiler_file_line(gboolean focus_editor)
{
	...
	if (gtk_tree_selection_get_selected(selection, &model, &iter))
	{
		...
		if (color == NULL || (!gdk_color_equal(color, &color_error_light) && !gdk_color_equal(color, &color_error_dark)))
		{
			if (color != NULL)
