...
static void init_builtin_filetypes(void)
{
	...
	FT_INIT( BASIC,      FREEBASIC,    "AGK",              NULL,                      SOURCE_FILE, NULL,                         COMPILED );
...
on_filetype_change(GtkCheckMenuItem *menuitem, gpointer user_data)
{
	...
	{
		/*
		if ( ((GeanyFiletype*)user_data)->id == GEANY_FILETYPES_BASIC )
		{
		...
		{
			scintilla_send_message( doc->editor->sci, SCI_AUTOCSETIGNORECASE, 0, 0 );
			scintilla_send_message( doc->editor->sci, SCI_AUTOCSETCASEINSENSITIVEBEHAVIOUR, SC_CASEINSENSITIVEBEHAVIOUR_RESPECTCASE, 0 );
		}
		*/
	}
