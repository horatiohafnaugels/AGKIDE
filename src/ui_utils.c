...
void ui_project_buttons_update(void)
{
	update_build_menu3();
}
...
void ui_sidebar_show_hide(void)
{
	...
	ui_widget_show_hide(gtk_notebook_get_nth_page(
		GTK_NOTEBOOK(main_widgets.sidebar_notebook), 1), interface_prefs.sidebar_symbol_visible);
	ui_widget_show_hide(gtk_notebook_get_nth_page(
		GTK_NOTEBOOK(main_widgets.sidebar_notebook), 0), interface_prefs.sidebar_openfiles_visible);
}
...
static void recent_project_activate_cb(GtkMenuItem *menuitem, G_GNUC_UNUSED gpointer user_data)
{
	...
	if (!project_find_by_filename(locale_filename) && project_load_file_with_session(locale_filename))
...
void ui_add_recent_document(GeanyDocument *doc)
{
	...
	rdata.app_name = (gchar *) "agk";
	rdata.app_exec = (gchar *) "agk %u";
...
{
	g_object_set(entry, "secondary-icon-stock", GTK_STOCK_CLOSE,
